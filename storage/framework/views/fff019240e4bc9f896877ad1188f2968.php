...
<?php $__env->startSection('title', 'Detail Kasus ' . $case->case_number); ?>

<?php $__env->startSection('content'); ?>
<div class="max-w-5xl mx-auto" x-data="caseDetail(<?php echo e($case->id); ?>)">
  ...
  <nav class="text-sm text-gray-500 mb-5">
    ...
    <span class="font-mono text-gray-900 font-medium"><?php echo e($case->case_number); ?></span>
  </nav>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
    ...
    <div class="font-mono text-lg font-bold text-blue-700 mb-3"><?php echo e($case->tracking_token); ?></div>
    ...
    <div class="mt-4 flex items-center gap-2 text-sm text-gray-600">
      ...
      <span>Dibuat</span>
      <span class="text-gray-800 font-medium"><?php echo e($case->created_at->translatedFormat('d M Y, H:i')); ?></span>
    </div>

    <?php if($case->submitted_at): ?>
    <div class="mt-2 flex items-center gap-2 text-sm text-gray-600">
      <i class="far fa-paper-plane text-gray-400"></i>
      <span>Dikirim</span>
      <span class="text-gray-800 font-medium"><?php echo e($case->submitted_at->translatedFormat('d M Y, H:i')); ?></span>
    </div>
    <?php endif; ?>
  </div>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
    ...
    <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
      <div>
        ...
        <dd class="font-mono text-base font-semibold text-gray-900"><?php echo e($case->case_number); ?></dd>
      </div>
      <div>
        <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Pemohon</dt>
        <dd class="text-base font-medium text-gray-900"><?php echo e($case->submitter?->name); ?></dd>
      </div>
      <div>
        <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Institusi</dt>
        <dd class="text-base text-gray-900"><?php echo e($case->institution?->name); ?></dd>
      </div>
      <div>
        <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Status</dt>
        <dd class="text-base text-gray-900"><?php echo e($status['label']); ?></dd>
      </div>
    </dl>
  </div>

  
  <?php if($case->petitioner_name || $case->petitioner_nik): ?>
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
    ...
    <div class="p-6">
      <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
        <?php if($case->petitioner_nik): ?>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">NIK</dt>
          <dd class="font-mono text-base font-semibold text-gray-900"><?php echo e($case->petitioner_nik); ?></dd>
        </div>
        <?php endif; ?>
        <?php if($case->petitioner_name): ?>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Nama Lengkap</dt>
          <dd class="text-base font-medium text-gray-900"><?php echo e($case->petitioner_name); ?></dd>
        </div>
        <?php endif; ?>
        <?php if($case->petitioner_phone): ?>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Nomor Telepon</dt>
          <dd class="font-mono text-base text-gray-900"><?php echo e($case->petitioner_phone); ?></dd>
        </div>
        <?php endif; ?>
      </dl>
    </div>
    ...
    <div class="p-6">
      <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5">
        <div>
          ...
          <dd class="text-base text-gray-900"><?php echo e($case->spouse_name ?? '-'); ?></dd>
        </div>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">NIK Mantan Pasangan</dt>
          <dd class="font-mono text-base text-gray-900"><?php echo e($case->spouse_nik ?? '-'); ?></dd>
        </div>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Tanggal Cerai</dt>
          <dd class="text-base text-gray-900"><?php echo e($case->divorce_date?->translatedFormat('d F Y') ?? '-'); ?></dd>
        </div>
        <div>
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Nomor Putusan PA</dt>
          <dd class="text-base text-gray-900"><?php echo e($case->verdict_number ?? '-'); ?></dd>
        </div>
        <?php if($case->notes): ?>
        <div class="col-span-2">
          <dt class="text-xs text-gray-500 uppercase tracking-wide mb-1.5">Catatan</dt>
          <dd class="text-sm text-gray-700 bg-gray-50 rounded-lg p-4 whitespace-pre-wrap leading-relaxed"><?php echo e($case->notes); ?></dd>
        </div>
        <?php endif; ?>
      </dl>
    </div>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
    <div class="bg-gradient-to-r from-emerald-50 to-emerald-100/50 px-6 py-4 border-b border-emerald-200 flex items-center justify-between">
      <h3 class="font-semibold text-gray-800 text-base">Dokumen Diunggah (<?php echo e($case->documents->count()); ?>)</h3>
      <?php if(in_array($case->status, ['DRAFT','SUBMITTED']) && auth()->user()->hasAnyRole(['pa_assistant','pa_staff'])): ?>
      <a href="<?php echo e(route('dashboard.upload')); ?>" class="text-sm text-emerald-600 hover:text-emerald-700 font-medium">
        <i class="fas fa-upload mr-1"></i>Upload Dokumen
      </a>
      <?php endif; ?>
    </div>
    <ul class="divide-y divide-gray-100">
      <?php $__empty_1 = true; $__currentLoopData = $case->documents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <li class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition">
          <div class="flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl <?php echo e($doc->mime_type === 'application/pdf' ? 'bg-red-100' : 'bg-blue-100'); ?> flex items-center justify-center flex-shrink-0">
              <i class="fas fa-<?php echo e($doc->mime_type === 'application/pdf' ? 'file-pdf text-red-500' : 'image text-blue-500'); ?> text-lg"></i>
            </div>
            <div>
              <div class="text-sm font-semibold text-gray-900 mb-0.5 flex items-center gap-2">
                ...
                <?php if($doc->status): ?>
                  ...
                  ?>
                  <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php echo e($docStatus['bg']); ?> <?php echo e($docStatus['text']); ?>">
              ...
              <div class="text-xs text-gray-500">
                ...
                <span class="text-gray-400"><?php echo e(number_format($doc->size_bytes / 1024, 1)); ?> KB</span>
                <?php if($doc->ocrResult): ?>
                <span class="mx-1">·</span>
                <a href="<?php echo e(route('dashboard.ocr.result', $doc->id)); ?>" class="text-blue-600 hover:text-blue-700">
          ...
          <div class="flex items-center gap-3">
            ...
            <a href="<?php echo e(route('dashboard.ocr.result', $doc->id)); ?>" 
  ...
  <?php if(auth()->user()->hasAnyRole(['pa_management', 'super_admin']) && $case->ocrValidations->isNotEmpty()): ?>
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
    <div class="bg-gradient-to-r from-indigo-50 to-indigo-100/50 px-6 py-4 border-b border-indigo-200 flex items-center justify-between">
      <h3 class="font-semibold text-gray-800 text-base">
        ...
        <span class="ml-2 text-xs font-normal text-gray-600">(<?php echo e($case->ocrValidations->count()); ?> dokumen)</span>
      </h3>
      <a href="<?php echo e(route('dashboard.review.show', $case->id)); ?>" 
    ...
    <div class="divide-y divide-gray-100">
      <?php $__currentLoopData = $case->ocrValidations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $validation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="p-6 hover:bg-gray-50/50 transition">
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="text-sm font-semibold text-gray-900">
                  <?php echo e($validation->document ? strtoupper(str_replace('_', ' ', $validation->document->document_type)) : 'Unknown'); ?>

                </div>
                <div class="text-xs text-gray-500">
                  <?php echo e($validation->created_at->translatedFormat('d M Y, H:i')); ?>
            ...
            <div class="flex items-center gap-2">
              ...
              ?>
              <span class="px-3 py-1 rounded-full text-xs font-medium <?php echo e($statusBadge['bg']); ?> <?php echo e($statusBadge['text']); ?>">
              ...
              <div class="flex items-center gap-2 bg-gray-50 px-3 py-1 rounded-full">
                ...
                <span class="text-sm font-bold <?php echo e($validation->overall_match_score >= 95 ? 'text-green-600' : ($validation->overall_match_score >= 80 ? 'text-yellow-600' : 'text-red-600')); ?>">
                  <?php echo e(number_format($validation->overall_match_score, 1)); ?>%
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 rounded-xl p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <?php $__currentLoopData = $displayFields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $fieldKey => $fieldLabel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="text-sm <?php echo e($isMismatch ? 'text-red-600 font-medium' : 'text-gray-900'); ?> truncate">
                      ...
                      <div class="flex-shrink-0 text-right">
                        <div class="text-xs font-bold <?php echo e($isMatch ? 'text-green-600' : ($isMismatch ? 'text-red-600' : 'text-yellow-600')); ?>">
                          <?php echo e(number_format($matchScore, 0)); ?>%
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <?php if($validation->is_reviewed): ?>
            <div class="mt-4 flex items-start gap-3 bg-green-50 border border-green-200 rounded-lg p-3">
              <i class="fas fa-check-circle text-green-600 mt-0.5"></i>
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-green-800">
                  Sudah Direview - <?php echo e(ucfirst($validation->review_action)); ?>

                </div>
                <div class="text-xs text-green-700 mt-1">
                  <span class="font-medium"><?php echo e($validation->reviewer->name ?? 'Unknown'); ?></span>
                  · <?php echo e($validation->reviewed_at?->translatedFormat('d M Y, H:i') ?? '-'); ?>

                </div>
                <?php if($validation->review_notes): ?>
                  <div class="text-xs text-green-700 mt-2 italic">
                    "<?php echo e($validation->review_notes); ?>"
                  </div>
                <?php endif; ?>
              </div>
            </div>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-5">
    ...
    <ol class="relative border-l border-gray-200 space-y-4 ml-3">
      <?php $__empty_1 = true; $__currentLoopData = $case->transitions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      <li class="ml-4">
        <span class="absolute -left-1.5 mt-1 w-3 h-3 bg-blue-600 rounded-full border-2 border-white"></span>
        <div>
          <p class="text-xs text-gray-500"><?php echo e($t->created_at->translatedFormat('d M Y, H:i')); ?></p>
          <p class="text-sm font-medium mt-1">
            <span class="text-gray-500"><?php echo e($t->from_state); ?></span>
            <i class="fas fa-arrow-right text-xs mx-1 text-gray-400"></i>
            <span class="text-blue-700"><?php echo e($t->to_state); ?></span>
          </p>
          <?php if($t->reason): ?>
          <p class="text-xs text-gray-600 mt-1 italic bg-gray-50 rounded px-2 py-1 inline-block">"<?php echo e($t->reason); ?>"</p>
          <?php endif; ?>
          <p class="text-xs text-gray-500 mt-1">oleh <?php echo e($t->actor?->name); ?></p>
        </div>
      </li>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
    ...
    <div class="flex flex-col sm:flex-row gap-3">
      ...
      <?php if($case->status === 'DRAFT' && auth()->user()->id === $case->submitter_id): ?>
      ...
      <?php if($case->status === 'PA_REVIEW' && auth()->user()->hasAnyRole(['pa_management','pa_staff'])): ?>
      ...
      <?php if($case->status === 'DISDUKCAPIL_VALIDATION' && auth()->user()->hasRole('disdukcapil_staff')): ?>
      ...
      <?php if(!in_array($case->status, ['COMPLETED', 'REJECTED'])): ?>
        <?php if(!$case->status === 'DRAFT' && !$case->status === 'PA_REVIEW' && !$case->status === 'DISDUKCAPIL_VALIDATION'): ?>
          <div class="text-sm text-gray-500 text-center py-3">
            Status saat ini: <span class="font-semibold"><?php echo e($status['label']); ?></span>
          </div>
        <?php endif; ?>
      <?php endif; ?>
    </div>
...
function caseDetail(caseId) {
  return {
    ...
    async rejectCase() {
      ...
      const endpoint = '<?php echo e(in_array($case->status, ["PA_REVIEW"]) ? "/api/v1/review/pa" : "/api/v1/review/disdukcapil"); ?>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\ProyekTA\resources\views/dashboard/cases/show.blade.php ENDPATH**/ ?>