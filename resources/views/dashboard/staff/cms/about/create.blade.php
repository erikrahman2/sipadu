...
@section('title', $editing ? 'Edit Section Tentang' : 'Tambah Section Tentang')
@section('page-title', $editing ? 'Edit Section Tentang' : 'Tambah Section Tentang')
...
@section('content')
<div class="max-w-3xl">
  <a href="{{ route('dashboard.admin.cms.kelola-konten.index', ['tab' => 'tentang']) }}" class="text-sm text-primary hover:underline mb-4 inline-block">
    <i class="fas fa-arrow-left mr-1"></i> Kembali
  </a>

  <form method="POST" action="{{ $editing ? route('dashboard.admin.cms.kelola-konten.about.update', $section) : route('dashboard.admin.cms.kelola-konten.about.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
    @csrf
    ...
    <div class="p-4 space-y-4">
      ...
      <div>
        ...
        <input type="text" name="content_type" value="{{ old('content_type', $section->content_type ?? '') }}" {{ $editing ? 'readonly disabled class="bg-gray-100"' : 'required' }}
          ...
          placeholder="tentang_sipadu, visi_misi, fitur_keunggulan, dll.">
    ...
    <div class="px-4 py-4 bg-gray-50 flex items-center justify-end gap-3">
      <a href="{{ route('dashboard.admin.cms.kelola-konten.index', ['tab' => 'tentang']) }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">Batal</a>
