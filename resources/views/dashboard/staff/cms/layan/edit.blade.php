...
@section('breadcrumb')
  ...
  <i class="fas fa-chevron-right text-xs"></i>
  <span class="text-stone-800 font-medium">Edit {{ $layan->nama }}</span>
@endsection
...
<div class="max-w-3xl">
  ...
  <form method="POST" action="{{ route('dashboard.admin.cms.layan.update', $layan) }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow-sm border border-stone-200 overflow-hidden">
    @csrf @method('PATCH')

    <div class="p-5 space-y-5">
      <div>
        <label class="block text-sm font-medium text-darktext mb-1.5">Nama Layanan <span class="text-red-500">*</span></label>
        <input type="text" name="nama" value="{{ old('nama', $layan->nama) }}" required
      ...
      <div>
        ...
        <textarea name="deskripsi" rows="5"
          ...
          placeholder="Penjelasan singkat tentang layanan">{{ old('deskripsi', $layan->deskripsi) }}</textarea>
      ...
      <div>
        ...
        <input type="text" name="icon" value="{{ old('icon', $layan->icon) }}"
      ...
      <div>
        ...
        <input type="number" name="urutan" value="{{ old('urutan', $layan->urutan) }}" min="0"
          class="w-full border border-stone-200 rounded-lg px-3.5 py-2.5 text-sm text-darktext placeholder-stone-400 focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none">
