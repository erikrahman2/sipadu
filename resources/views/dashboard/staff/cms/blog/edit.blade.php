...
@section('breadcrumb')
  ...
  <a href="{{ route('dashboard.admin.cms.kelola-konten.blog.index') }}" class="text-gray-500 hover:text-gray-700">Kelola Berita</a>
  <i class="fas fa-chevron-right text-xs"></i>
  <span class="text-gray-800 font-medium">Edit {{ Str::limit($post->title, 30) }}</span>
@endsection

@section('content')
<div class="max-w-3xl">
  <a href="{{ route('dashboard.admin.cms.kelola-konten.blog.index') }}" class="text-sm text-primary hover:underline mb-4 inline-block">
    <i class="fas fa-arrow-left mr-1"></i> Kembali
  </a>

  <form method="POST" action="{{ route('dashboard.admin.cms.kelola-konten.blog.update', $post) }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
    @csrf @method('PATCH')
    ...
    <div class="p-4 space-y-4">
      ...
      <div>
        ...
        <textarea name="excerpt" rows="2"
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none resize-none">{{ old('excerpt', $post->excerpt) }}</textarea>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Konten <span class="text-red-400">*</span></label>
        <textarea name="content" rows="10" required
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none resize-y font-mono">{{ old('content', $post->content) }}</textarea>
      </div>
      ...
      <div>
        ...
        @if($post->cover_image)
        <div class="mb-2">
          <img src="{{ Storage::url($post->cover_image) }}" class="h-24 rounded-lg border" alt="Cover">
        </div>
        @endif
      ...
      <p class="text-xs text-gray-400">
        Dipublikasi: {{ $post->published_at ? $post->published_at->format('d/m/Y H:i') : '—' }}<br>
        URL: <a href="{{ route('berita') }}/{{ $post->slug }}" target="_blank" class="text-primary underline">{{ route('berita') }}/{{ $post->slug }}</a>
      </p>
    </div>

    <div class="px-4 py-4 bg-gray-50 flex items-center justify-end gap-3">
      <a href="{{ route('dashboard.admin.cms.kelola-konten.blog.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">Batal</a>
