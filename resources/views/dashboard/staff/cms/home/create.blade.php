...
@section('breadcrumb')
  ...
  <i class="fas fa-chevron-right text-xs"></i>
  <span class="text-gray-800 font-medium">Tambah Section</span>
@endsection

@section('content')
<div class="max-w-3xl">
  <a href="{{ route('dashboard.admin.cms.kelola-konten.index', ['tab' => 'home']) }}" class="text-sm text-primary hover:underline mb-4 inline-block">
    <i class="fas fa-arrow-left mr-1"></i> Kembali
  </a>

  <form method="POST" action="{{ route('dashboard.admin.cms.kelola-konten.home.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden divide-y divide-gray-100">
    @csrf

    <div class="p-4 space-y-4">
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Kunci Section <span class="text-red-400">*</span></label>
        <input type="text" name="section_key" value="{{ old('section_key') }}" required
          class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none"
          placeholder="contoh: hero, services, cta_block">
        <p class="text-xs text-gray-400 mt-1">Hanya huruf kecil, angka, dan underscore. Tidak bisa diubah setelah disimpan.</p>
        @error('section_key') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
      </div>
      ...
      <div>
        ...
        <textarea name="content" rows="6"
          ...
          placeholder="HTML didukung">{{ old('content') }}</textarea>
      </div>
    ...
    <div class="px-4 py-4 bg-gray-50 flex items-center justify-end gap-3">
      <a href="{{ route('dashboard.admin.cms.kelola-konten.index', ['tab' => 'home']) }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800 transition">Batal</a>
